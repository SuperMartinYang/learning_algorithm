...
def algorithm1(problem, trace = None):
    ...
    # get a list of all location in the dividing column
    divider = crossProduct(range(problem.numRow), [mid])

    # find the maximun in the dividing column
    bestLoc = problem.getMaximun(divider, trace)

    # see if the maximun vale we fond on the dividing line has a better
    ...
    # otherwise, figure out which subproblem contains the neighbor, and recurse in that half
...
def crossProduct(list1, list2):
    """
    Returns all pairs with one item from the first list and one item from the second list
