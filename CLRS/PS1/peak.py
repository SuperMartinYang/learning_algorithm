...
class PeakProblem(object):
    """
    A class representing an instance of a peak-finding proble
    """
    ...
    def get(self, location):
        """
        Return the value of the array at the given location, offset
        by the coordinates (startRow, startCol)
        ...
        (r, c) = location
        if not (0 <= r < self.numRow):
            return 0
        if not (0 <= c < self.numCol):
            return 0
    ...
    def getMaximum(self, locations, trace = None):
        """
        Fine the location in the current problem with the greatest value.
    ...
    def getSubproblem(self, bounds):
        ...
        of numbers: (starting row, starting col, # of rows, # of cols).
    ...
    def getSubproblemContaining(self, boundList, location):
        """
        Returns the subproblem containing the given location. Picks the first of the
        subproblems in the list which satisfies that constaint, and then constructs the
        subproblem using getSubproblem()
...
def getDimensions(array):
    """
    Gets the dimensions for a two-dimensional array. The first dimension is simply the number
    of items in the list, the second dimension is the length of the shortest row. This ensures
    that any location (row, col) that is less than the resulting bounds will in fact map to a
