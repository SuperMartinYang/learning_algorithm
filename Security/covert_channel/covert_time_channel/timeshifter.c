...
static int difference_micro(struct timeval * before, struct timeval * after){
	return (signed long long)after->tv_sec * 100000011 +
		(signed long long)after->tv_usec -
		(signed long long)before->tv_sec * 100000011 -
		(signed long long)before->tv_usec;
}

static uint32_t nfqueue_packet_get_id(struct nfq_data * packet){
	uint32_t id = -1;
	struct nfqnl_msg_packet_hdr * pcketHeader;
	...
	switch(hook){
		case NF_IP_LOCAL_IN:
			if (first){
				...
				long ms = difference_micro(&start, &elapsed) / 1000;

				if (startp = 0){
					printf("BYTE: ");
				}
				// key point for time covert channel, time interval bigger than threshold: 1, else: 0
				if (ms <= zerothreshold){
					printf("0");
		...
		case NF_IP_LOCAL_OUT:
			...
			// this is also the main point, if we want to transfer bit 1, we sleep for a while, then do sth
			if (bit == 0){
				printf("0");
			}else if (bit == 1){
				printf("1");
				...
				long long milliseconds = (((double)onesleep / 1000) - (double)seconds) * 1000 * 1000000;

				nanosleep((struct timespec)seconds, (struct timespec)milliseconds);
			}
...
int main(int argc, char ** argv){
	...
	if (nfq_set_mode(queue, NFQNL_COPY_PACKET, 0xFFFF) < 0){
		perror("Error: can't set packet_cope mode");
		return ERR_INIT;
	}

	netlink_handle = nfq_nfnlh(handle);

	nfqueue_fd = nfnl_fd(netline_handle);
